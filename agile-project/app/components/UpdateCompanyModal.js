...
const UpdateCompanyModal = ({ isOpen, onClose, company, setCompany }) => {
  ...
  const handleUpdateCompany = async () => {
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        const response = await axios.put(`/api/companies/${company._id}`, formData);
